...
public class PlayerHealth : MonoBehaviour
	...
	void Update() {
		...
		if (timer >= timeAfterWeLastTookDamage) {
			if (!(healthSliderBackground == null)) healthSliderBackground.value = Mathf.Lerp(healthSliderBackground.value, healthSliderForeground.value, 2 * Time.deltaTime);
		}
	...
	void Death() {
		...
		if (deathClip && playerAudio) playerAudio.clip = deathClip;
		if (playerAudio) playerAudio.Play();
		...
		*/
		PlayerController XPC = GetComponentInParent<PlayerController>(); 
		//if (XPC.goHamsterBallModeRoot.activeSelf)
		//{
			//XPC.SwitchToFirstPersonRodentMode();
		//}
		//else if(XPC.goFPRodentModeRoot.activeSelf)
		//{
			//XPC.SwitchToFreeCameraMode();
			//XA.LoadTitleMenu();
		//}
		//public GameObject goHamsterBallModeRoot;
